...
const { UserVehicle, VehicleRTOData } = require('../models');
const { parsePagination, buildMeta } = require('../utils/pagination');

/**
 * GET /api/rto-data?page=1&limit=300
 * Returns RTO data for ACTIVE vehicles of the authenticated client.
...
router.get('/', async (req, res) => {
  try {
    ...
    if (vehicleNumbers.length) {
      data = await VehicleRTOData.findAll({
    ...
    console.error('[api/rto-data] error:', err);
